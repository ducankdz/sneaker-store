...
$(document).ready(function () {
    ...
    if ( typeof noUiSlider === 'object' ) {
        ...
        document.getElementById('filter-button').addEventListener('click', function() {
            ...
            var minPrice = parseInt(values[0].replace('đ', '').replace(/,/g, ''));
            var maxPrice = parseInt(values[1].replace('đ', '').replace(/,/g, ''));

            fetchProductsByPrice(minPrice, maxPrice);
        });
	}

    function fetchProductsByPrice(minPrice, maxPrice) {
        var url = `/products/price?minPrice=${minPrice}&maxPrice=${maxPrice}`;

        // Navigate to the URL to reload the entire page
